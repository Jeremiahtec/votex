...
export default function Results() {
  ...
  if (!data) return (
    <div className="pt-32 px-4 flex flex-col items-center animate-fade-in">
      ...
      <h2 className="text-2xl font-black text-slate-900 dark:text-white mb-2">Poll not found</h2>
  ...
  return (
    <div className="pt-24 pb-12 px-4 sm:px-6 lg:px-8 max-w-3xl mx-auto animate-fade-in">
      <div className="mb-8 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-xs font-bold text-slate-500 bg-white dark:bg-surface-elevated px-4 py-2 rounded-full shadow-sm border border-slate-200 dark:border-slate-800">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-6 mb-10 pb-8 border-b border-slate-100 dark:border-slate-800">
            <div>
              <div className="flex gap-2 items-center mb-3">
                <span className="text-xs text-primary-600 dark:text-primary-400 uppercase tracking-widest font-black">Final Results</span>
                {data.anonymous && (
              ...
              <h1 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white leading-snug mb-6">
                {data.title}
              </h1>
            ...
            <div className="flex flex-col items-start sm:items-end shrink-0 bg-slate-50 dark:bg-surface-dark p-4 rounded-xl border border-slate-100 dark:border-slate-800">
              <div className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-br from-primary-500 to-purple-600">
                {data.totalVotes}
              </div>
              ...
              return (
                <div key={opt.id} className="relative group">
                  <div className="flex justify-between items-end mb-2 relative z-10 px-1">
                    <span className={`font-semibold flex items-center ${isWinner ? 'text-slate-900 dark:text-white font-black' : 'text-slate-700 dark:text-slate-300'}`}>
                      {opt.text}
                      {isWinner && <Trophy className="w-4 h-4 ml-2 text-amber-500 fill-amber-500" />}
                    </span>
                    <div className="text-right">
                      <span className="font-black text-lg text-slate-900 dark:text-white">{opt.percentage}%</span>
                    ...
                    <div 
                      ...
                      style={{ 
                        ...
                        background: isWinner 
                          ...
                          : 'var(--color-border)',
                        backgroundColor: !isWinner && '#94a3b8'
                      }}
                    >
